...
G4VPhysicalVolume* MyDetectorConstruction::Construct()
{
   ...
   std::vector<G4double> v_energy{1.239841939*eV/0.2,1.239841939*eV/0.9};
   std::vector<G4double> v_rindexAerogel{1.1, 1.1};
   std::vector<G4double> v_rindexWorld{1.1, 1.1};

   G4MaterialPropertiesTable *mptAerogel = new G4MaterialPropertiesTable();
   //mptAerogel->AddProperty("RINDEX",energy,rindexAerogel,2);
   mptAerogel->AddProperty("RINDEX",v_energy,v_rindexAerogel);
   ...
   mptWorld->AddProperty("RINDEX", v_energy, v_rindexWorld);
   ...
   G4VPhysicalVolume * physRadiator = new G4PVPlacement(0,G4ThreeVector(0.,0.,0.25*m),logicalRadiator,"physRadiator",logicalWorld,false,0,true);
